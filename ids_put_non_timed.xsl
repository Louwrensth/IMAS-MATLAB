...
<xsl:template match="IDS" mode="PUT_NON_TIMED">
  <xsl:result-document href="src/ids/put_non_timed_{@name}.c.in" standalone="yes" method="text">
    ...
    if (pids_props == NULL)
      mexErrMsgIdAndTxt("IMAS:ids_put_non_timed:invalid_ids_properties",
      "Unable to retrieve ids%%ids_properties");
    phomog_time = mxGetField(pids_props, (mwIndex) 0, "homogeneous_time");
    if (phomog_time == NULL)
      mexErrMsgIdAndTxt("IMAS:ids_put_non_timed:invalid_homogeneous_time",
    ...
    {
    mexWarnMsgIdAndTxt("IMAS:ids_put_non_timed:empty_ids", "IDS <xsl:value-of select="@name"/> is found to be EMPTY (homogeneous_time undefined). PUTNONTIMED quits with no action.");
    return 0;
    }
    if(idx &lt; 1)
