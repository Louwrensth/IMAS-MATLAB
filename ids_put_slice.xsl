 ...
 <xsl:result-document href="src/ids/ids_put_slice.h" standalone="yes" method="text">
  ...
  <xsl:result-document href="src/ids/put_slice_ids.c" standalone="yes" method="text">
    ...
    if (getOpCtx > 0) {
      ...
      if (status.code >= 0) status.code = status_end.code; /* Result of hli_end_action is only relevant if there was no error before */
    }
    
    if (sliceOp) {
      /* Open putSlice context */
      if (status.code >= 0) status = hli_begin_slice_action(expIdx, idsFullName, WRITE_OP, UNDEFINED_TIME, UNDEFINED_INTERP, &amp;putSliceOpCtx);
