...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.get_int_as_double));

  // params.put_int_from_double
  ifield = mxAddField(plhs[0], "put_int_from_double");
  if (ifield < 0)
    mexErrMsgIdAndTxt("IMAS:imas_mex_get_params:internal","Unable to create structure params");
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.put_int_from_double));

  // params.get_empty_as_nan
  ifield = mxAddField(plhs[0], "get_empty_as_nan");
  if (ifield < 0)
    mexErrMsgIdAndTxt("IMAS:imas_mex_get_params:internal","Unable to create structure params");
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.get_empty_as_nan));

  // params.put_empty_from_nan
  ifield = mxAddField(plhs[0], "put_empty_from_nan");
  if (ifield < 0)
    mexErrMsgIdAndTxt("IMAS:imas_mex_get_params:internal","Unable to create structure params");
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.put_empty_from_nan));

  // params.use_cell_array_for_array_of_structures
  ifield = mxAddField(plhs[0], "use_cell_array_for_array_of_structures");
  if (ifield < 0)
    mexErrMsgIdAndTxt("IMAS:imas_mex_get_params:internal","Unable to create structure params");
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.use_cell_array_for_array_of_structures));
  ...
  if (ifield < 0)
    mexErrMsgIdAndTxt("IMAS:imas_mex_get_params:internal","Unable to create structure params");
  mxSetFieldByNumber(plhs[0], 0, ifield, mxCreateDoubleScalar((double) params.error_on_missing_field));
