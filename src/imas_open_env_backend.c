...
/**
   \file imas_open_env_backend.c
   open IMAS database using the specified backend id in MATLAB External Interfaces
   
   This is a MEX file for MATLAB.

   Usage:
   \code{.m} 
   idx = imas_open_env_backend(name, shot, run, user, tokamak, version, backend_id)
   \endcode

   MATLAB help:
   \include matlab/imas_open_env_backend.m
 */
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    if (nrhs != 7) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:nargin", "Seven inputs required.");
    }
    /* make sure the 1st input argument is a string */
    if (!mxIsChar(prhs[0])) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:notChar", "Input name must be a string.");
    }
    /* make sure the 2nd input argument is scalar */
    if (!mxIsNumeric(prhs[1]) || !mxIsScalar(prhs[1])) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:notScalar", "Input shot must be a scalar.");
    }
    /* make sure the 3rd input argument is scalar */
    if (!mxIsNumeric(prhs[2]) || !mxIsScalar(prhs[2])) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:notScalar", "Input run must be a scalar.");
    }
    /* make sure the 4th input argument is a string */
    if (!mxIsChar(prhs[3])) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:notChar", "Input user must be a string.");
    }
    /* make sure the 5th input argument is a string */
    if (!mxIsChar(prhs[4])) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:notChar", "Input tokamak must be a string.");
    }
    ...
    if (nlhs > 1) {
        mexErrMsgIdAndTxt("IMAS:imas_open_env_backend:nargout", "One output maximum required.");
    }
