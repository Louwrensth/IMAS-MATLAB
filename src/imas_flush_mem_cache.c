...
/*
 * imas_flush_mem_cache.c - flush cached memory for IMAS database in MATLAB External Interfaces
 *
 *              idx = imas_flush_mem_cache(idx)
 *
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    if (nrhs != 1) {
        mexErrMsgIdAndTxt("IMAS:imas_flush_mem_cache:nargin", "One input required.");
    }
    // make sure the 1st input argument is scalar
    if (!mxIsNumeric(prhs[0]) || !mxIsScalar(prhs[0])) {
        mexErrMsgIdAndTxt("IMAS:imas_flush_mem_cache:notScalar", "Input idx must be a scalar.");
    }
